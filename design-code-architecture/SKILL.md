...
description: 'Guided journey for designing a new app architecture: boundaries, domain model, data choices, resilience, and scope. Orchestrates clean-architecture, domain-driven-design, system-design, ddia-systems, software-design-philosophy, release-it, pragmatic-programmer, and 37signals-way. Use when the user wants to structure a new app, choose a modular monolith versus services, define bounded contexts, select storage and consistency models, or design reliability before implementation. Records decisions in ARCHITECTURE.md, RELIABILITY.md, TECH-DEBT.md, TESTING.md, and DESIGN-CODE-ARCHITECTURE-PLAN.md so the journey can resume across sessions. For an existing codebase, use remove-technical-debt or improve-code-quality; for an unvalidated idea, use create-business or create-app. Invoke an individual framework skill for a narrow question.'
license: MIT
metadata:
  author: wondelai
  version: "1.0.0"
